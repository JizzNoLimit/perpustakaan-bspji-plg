...
export default function PinjamIndex({ token }) {
    ...
    async function getPinjam() {
        const req = await fetch(process.env.API_SERVER + `/admin/pinjam?search_query=${keyword}&page=${page}&limit=${limit}`, {
            method: 'GET',
            headers: {
    ...
    // Handle pencarian anggota|| pinjam 
    function handleSearch(e) {
        e.preventDefault()
        setKeyword(e.target[0].value)
    }
    function converDate(day, dd, mm, yy) {
        const hari = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', "Jum\'at", 'Sabtu']
        const bulan = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"]

        return (
            <span className="text-sm"><strong>{hari[day]}</strong>, {dd} {bulan[mm]} {yy}</span>
        )
    }
        ...
        <AdminLayout>
            ...
            <div className="screen-bersih overflow-y-scroll srcool-hiden  relative w-full">
                <div className="flex items-center mb-3 space-x-2">
                    ...
                    <button onClick={() => setAddActive(true)} className="btn text-sm w-auto">Pinjam</button>
                </div>
                <table className="w-full text-sm">
                    <thead className="">
                        <tr className="text-left">
                            ...
                            <th className="w-[200px] px-4 py-5 bg-white text-center">Deadline</th>
                            ...
                            pinjam.map((data, index) => (
                                <tr className="group hover:bg-gray-100 text-center" key={index}>
                                    ...
                                    <td className="p-4 text-center">
                                        <span className="">
                                            {converDate(
                                                new Date(data.batas_pinjam).getDay(),
                                                new Date(data.batas_pinjam).getDate(),
                                                new Date(data.batas_pinjam).getMonth(),
                                                new Date(data.batas_pinjam).getFullYear()
                                            )}
                                        </span>
                                    </td>
                                    <td className="p-4 text-center space-x-1 text-xs">
                                        <button onClick={() => editOnClick(data.id)} className="btn px-2">Detail</button>
                                        <button onClick={() => handleDelete(data.id, data.bukuId)} className="btn-delete px-2">Hapus</button>
                                    </td>
                                </tr>
                            ))
                        )}
                ...
                <div className={`${addActive ? 'right-0' : '-right-full'} fixed top-0 h-screen w-[340px] p-4 bg-white space-y-4 z-20 ease-out duration-500 transition-all`}>
                    ...
                    <form onSubmit={(e) => e.preventDefault(e)}>
                        <section>
                            <label htmlFor="userId" className="mx-2 text-sm">
                                No. Anggota
                            </label>
                            <input
                                ...
                                className="input"
                            />
                            <label htmlFor="bukuId" className="mx-2 text-sm">
                                No. Buku
                            </label>
                            <input
                                onChange={fieldHandler.bind(this)}
                ...
                <div className={`${updateActive ? 'right-0' : '-right-full'} fixed top-0 h-screen w-[340px] px-4 py-6 bg-white space-y-4 z-20 ease-out duration-500 transition-all`}>
                    ...
                    <div className="w-full h-[320px] p-4 text-sm bg-gray-200 rounded-2xl">
                        {/* <table>
                            <thead>
                                <tr>
                                ...
                                <tr>
                                    <th className="w-[70px] py-1 text-left">Pinjam</th>
                                        <td>: {converDate(
                                            new Date(feild.tgl_pinjam).getDay(),
                                ...
                                <tr>
                                    <th className="w-[70px] py-1 text-left">Deadline</th>
                                        <td>: {converDate(
                                            new Date(feild.batas_pinjam).getDay(),
                                ...
                                <tr>
                                    ...
                                    <tr>
                                        <th className="w-[70px] py-1 text-left">Durasi</th>
                                        <td>: <Countdown deadline={feild.batas_pinjam} />
                                        </td>
                                    </tr>
                            </thead>
                        </table> */}
                    ...
                    <div className="absolute space-y-2 bottom-10 right-4 left-4">
                        <button onClick={() => handleKembali(feild.id)} className="btn w-full">Dikembalikan</button>
